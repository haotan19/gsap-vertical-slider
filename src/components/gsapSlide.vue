  ...
  if (cardEl.value) {
    if ((oldPos === 2 && newPos === -2) || (oldPos === -2 && newPos === 2)) {
      ...
      gsap.to(cardEl.value, {
        ...
        rotateY: -30 * newPos,
        filter: newPos === 0 ? "grayscale(0) brightness(1)" : "grayscale(100) brightness(.5)",
        onStart: () => {
          data.onPosTransition = true
        },
        onComplete: () => {
...
onMounted(() => {
  if (cardEl.value && cardEl.value) {
    gsap.to(cardEl.value, {
      ...
      filter: props.pos === 0 ? "grayscale(0) brightness(1)" : "grayscale(100) brightness(.5)",
      onStart: () => {
...
const getHeadingHTML = () => "<span>" + props.slideData.heading?.split("").join("</span><span>") + "</span>"

// TODO: 1. Find a set of vertical product ot display...
// TODO: 2. Find the font

const getImgUrl = ({ imgType = "bg" }: { imgType?: "fg" | "bg" } = {}) => {
  if (props.slideData.img && imgType === "bg") return new URL(props.slideData.img, import.meta.url).href
  else if (props.slideData.imgFg) return new URL(props.slideData.imgFg, import.meta.url).href
}
</script>

<template>
  <div ref="cardContainer" class="absolute-center w-[300px] h-[450px] slide-inner" @pointerleave="resetMouseOverEffect">
    <div ref="cardEl" class="w-full h-full">
      <img class="w-full h-full object-cover" :src="getImgUrl()" :alt="slideData.heading" />
      <img v-if="getImgUrl({ imgType: 'fg' })" ref="fgEl" class="absolute top-0" :src="getImgUrl({ imgType: 'fg' })" :alt="slideData.heading" style="transform-origin: top center 25px" />
    </div>
  </div>
  <div ref="contentEl" v-if="pos === 0 && slideData.heading" class="absolute left-1/2 top-1/2 pointer-events-none z-10" style="transform: translateX(-55%) translateY(80%)">
    <h1 class="text-5xl text-white uppercase font-bold max-w-sm" v-html="getHeadingHTML()"></h1>
  </div>
</template>
